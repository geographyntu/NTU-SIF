...
%
% Last modified:   2023/08/09   Clancy
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [zenith, azimuth, sunrise, sunset] = spa(year, month, day, hour, minute, second, timezone, ...
    longitude, latitude, elevation, pressure, temperature, slope, azm_rotation, to_print)
% clc
% clear
format long g
...
spa.delta_ut1     = 0;              % a fraction of a second, that is added to the UTC
spa.delta_t       = 69.22;          % the difference between the Earth rotation time and the Terrestrial Time (https://www.stjarnhimlen.se/comp/time.html)
spa.longitude     = longitude;
spa.latitude      = latitude;
spa.elevation     = elevation;
spa.pressure      = pressure;
spa.temperature   = temperature;
spa.slope         = slope;          % the slope of the surface
spa.azm_rotation  = azm_rotation;   % the surface azimuth rotation angle
spa.atmos_refract = 0.5667;         % the atmospheric refraction (typically 0.5667 at sunrise and sunset)
...
if (result == 0)  %check for SPA errors
    ...
    if to_print
        ...
        fprintf('L:             %.6e degrees\n',spa.l);             % the Earth heliocentric longitude
        fprintf('B:             %.6e degrees\n',spa.b);             % the geocentric latitude
        fprintf('R:             %.6f AU\n',spa.r);                  % the aberration correction
        fprintf('H:             %.6f degrees\n',spa.h);             % the observer local hour angle
    ...
    sunrise = sprintf('%02d:%02d:%02d', floor(spa.sunrise), floor(min), floor(sec));
    ...
    sunset = sprintf('%02d:%02d:%02d', floor(spa.sunset), floor(min), floor(sec));
